    ...
    try {
      ...
      const target =
        data.members.find(
          member =>
            member &&
            member.name === "TEST-030" &&
            member.status === "確認中"
        );

      if (!target) {
        return res.status(404).send(
          "TEST-030の確認中データが見つかりません。"
        );
      }
...
<p>
FOREVERへのFBO登録が確認され、<br>
あなた専用の<br>
Vera Sky Harmony（VSH）の利用が<br>
ここから始まります。
</p>
